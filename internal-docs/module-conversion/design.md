...
### Whether `transform` runs at all is decided by who loaded the module

`module_task.rs:284-302` runs `transform` for `StrOrBytes::Str` and skips it for
`StrOrBytes::Bytes`. Which arm a module lands in is not a property of its module
type; it is a property of _who produced its content_.

Configure `moduleTypes: { '.png': 'base64' }` and the core reads the file as
bytes (`load_source.rs:79-96`), so `transform` never fires. Add a plugin whose
`load` returns a string for that same `.png` and `load_source` takes the `Str`
branch, so `transform` fires. Same module, same module type, opposite behavior.

Today this is accidental: a plugin has no way to _choose_, because `load` cannot
return bytes. Once it can (below), the same dispatch becomes a rule the plugin
...
This is behavior-preserving for every module type except one. `base64`,
`dataurl` and `binary` already skip `transform`; `copy` never reached it, being
external at `resolve_id`; `json`, `text`, `empty` and the JS family stay
text-sourced. **`asset` is the exception**: today `AssetModulePlugin::load`
returns the placeholder _string_, so `load_source` yields `StrOrBytes::Str` and
...
This is accepted, on two grounds. It is unavoidable under the invariant — the
whole point is that an asset's content is its bytes, and `transform` cannot see
bytes. And nothing is expected to depend on it: `__ROLLDOWN_ASSET__#` is a
private detail of `AssetModulePlugin`, a `transform` hook receiving
...
content, because every AST span indexes into it and the codegen sourcemap builds
`sourcesContent` from it (`crates/rolldown/src/utils/render_ecma_module.rs:104-113`).
...
`asset`/`copy`. That is a breaking change for those types — today `code` holds
the _encoded_ string — but it breaks loudly rather than silently: a plugin doing
`info.code.includes(…)` gets a null-dereference instead of quietly matching
against base64 noise.
...
One consequence to know: `this.load` builds a `ResolvedId` with
`..Default::default()` (`native_plugin_context.rs:64-71`), so it never sees the
`asserted_module_type` an import edge carries — `with { type: … }`, or
`new URL()` ⇒ `Asset` (`new_url.rs:69`). A `.png` reached only by `new URL()`,
in a project with no `moduleTypes` entry for it, therefore comes back as `Js`
if `this.load` is what pulls it into the graph, and as `Asset` if something else
got there first. See Unresolved Questions.
...
emits the file and returns a **proxy** — a re-export from an external
placeholder:
...
The external import survives, `render_chunk` rewrites it as before, and
`this.load` sees the bytes. The cost is an extra graph node and a re-export
chain per copied file.
...
- **`require()` interop.** The snapshot above shows `require('./hello.txt')`
  surviving as a runtime `require` of the rewritten path. The proxy's re-export
  chain must lower to the same thing, not to a `__toESM`-wrapped namespace.
...
- **The copy proxy's three properties** — the copy design is settled (proxy
  module, for import order), but the properties listed under "What the proxy must
  get right" are asserted, not verified: that a side-effect-only proxy resists
  tree-shaking, that `require()` of a re-exported external still lowers to a
  runtime `require`, and that `export *` from an external behaves as intended in
  the linker. Each needs a test before the copy phase is committed to.
...
- **Edge-typed modules are not reliably `this.load`-able.** A module typed by its
  import edge gets that type from _how it was imported_, so what `this.load`
  returns depends on whether the module was already scanned. The caller can
  detect the mismatch via `ModuleInfo.moduleType` but cannot repair it. Either
  this is documented as a limitation, or `this.load` needs a way to reach the
  edge assertion — which runs into id-is-identity. Blocks the Vite CSS case for
  `new URL()` assets. (Related, pre-existing: two imports of one file with
  conflicting `with { type: … }` attributes hit the same `VisitState::Seen` early
  return, and the first silently wins.)
